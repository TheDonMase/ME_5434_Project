...
toler = 1.e-7; 	% Tolerance for iterative residual convergence
...
% Top boundary (Lid)
u(1:imax, jmax, 2) = uinf; % u-velocity
u(1:imax, jmax, 3) = zero; % v-velocity

% Bottom boundary
u(1:imax, 1, 2) = zero; % u-velocity
u(1:imax, 1, 3) = zero; % v-velocity

% Left boundary
u(1, 1:jmax, 2) = zero; % u-velocity
u(1, 1:jmax, 3) = zero; % v-velocity

% Right boundary
u(imax, 1:jmax, 2) = zero; % u-velocity
u(imax, 1:jmax, 3) = zero; % v-velocity

% Pressure boundary conditions using linear extrapolation
u(1:imax,1,1) = 2*u(1:imax,2,1) - u(1:imax,3,1); % Bottom boundary
u(1:imax,jmax,1) = 2*u(1:imax,jmax-1,1) - u(1:imax,jmax-2,1); % Top boundary

u(1,1:jmax,1) = 2*u(2,1:jmax,1) - u(3,1:jmax,1); % Left boundary
u(imax,1:jmax,1) = 2*u(imax-1,1:jmax,1) - u(imax-2,1:jmax,1); % Right boundary
...
for j = 2:jmax-1
    for i = 2:imax-1
        ...
        % Calculate lambda_x and lambda_y
        lambda_x = 0.5*(abs(u(i,j,2)) + sqrt(u(i,j,2)^2 + 4*beta2));
        lambda_y = 0.5*(abs(u(i,j,3)) + sqrt(u(i,j,3)^2 + 4*beta2));
        
        % Calculate |lambda|_max
        lambda_max = max([lambda_x, lambda_y]);
        
        % Calculate local convective time step restriction
        dtconv = min(dx, dy)/lambda_max;
        
        % Calculate diffusive stability limit for the time step
        dtvisc = (dx*dy)/(4*rmu/rho);
        
        % Combine the convective and diffusive time step limits and add the CFL number
        dt(i,j) = cfl*min([dtconv, dtvisc]);
        
        % Update global time step (max allowable in domain)
        dtmin = min([dt(i,j), dtmin]); %dtmin = min(dt(:));
    end
end

% Use dt for local time stepping
% Use dtmin for global time stepping
...
for j = 3:jmax-2
    for i = 3:imax-2
        ...
        % Calculate lambda_x and lambda_y
        lambda_x = max(0.5*(abs(u(i,j,2)) + sqrt(u(i,j,2)^2 + 4*beta2)));
        lambda_y = max(0.5*(abs(u(i,j,3)) + sqrt(u(i,j,3)^2 + 4*beta2)));
        
        % Calculate d4pdx4 and d4pdy4, 2nd order accurate difference
        ...
        % Calculate artviscx and artviscy
        artviscx(i,j) = ((-abs(lambda_x)*Cx*dx^3)/beta2) * d4pdx4;
        artviscy(i,j) = ((-abs(lambda_y)*Cy*dy^3)/beta2) * d4pdy4;
    end
end

for i = 2:imax-1
    % Bottom
    diffx = artviscx(i,2+2) - artviscx(i,2+1);
    artviscx(i,2) = artviscx(i,2+1) - diffx;

    diffy = artviscy(i,2+2) - artviscy(i,2+1);
    artviscy(i,2) = artviscy(i,2+1) - diffy;

    % Top
    diffx = artviscx(i,jmax-4) - artviscx(i,jmax-3);
    artviscx(i,jmax-2) = artviscx(i,jmax-3) - diffx;

    diffy = artviscy(i,jmax-4) - artviscy(i,jmax-3);
    artviscy(i,jmax-2) = artviscy(i,jmax-3) - diffy;
end

for j = 2:jmax-1
        % Left
        diffx = artviscx(2+2,j) - artviscx(2+1,j);
        artviscx(2,j) = artviscx(2+1,j) - diffx;

        diffy = artviscy(2+2,j) - artviscy(2+1,j);
        artviscy(2,j) = artviscy(2+1,j) - diffy;

        % Right
        diffx = artviscx(imax-4,j) - artviscx(imax-3,j);
        artviscx(imax-2,j) = artviscx(imax-3,j) - diffx;

        diffy = artviscy(imax-4,j) - artviscy(imax-3,j);
        artviscy(imax-2,j) = artviscy(imax-3,j) - diffy;
end
...
for j = 2:jmax-1
    for i = 2:imax-1
        dpdx = (uold(i+1,j,1)-u(i-1,j,1))/(2*dx);       % First derivative of pressure w.r.t. x
        dpdy = (uold(i,j+1,1)-u(i,j-1,1))/(2*dy);       % First derivative of pressure w.r.t. 

        dudx = (uold(i+1,j,2)-u(i-1,j,2))/(2*dx);       % First derivative of x velocity w.r.t. x
        dudy = (uold(i,j+1,2)-u(i,j-1,2))/(2*dy);       % First derivative of x velocity w.r.t. y

        dvdx = (uold(i+1,j,3)-u(i-1,j,3))/(2*dx);       % First derivative of y velocity w.r.t. x
        dvdy = (uold(i,j+1,3)-u(i,j-1,3))/(2*dy);       % First derivative of y velocity w.r.t. y

        d2udx2 = (uold(i+1,j,2)-2*uold(i,j,2)+u(i-1,j,2))/dx^2;     % Second derivative of x velocity w.r.t. x
        d2udy2 = (uold(i,j+1,2)-2*uold(i,j,2)+u(i,j-1,2))/dy^2;     % Second derivative of x velocity w.r.t. y

        d2vdx2 = (uold(i+1,j,3)-2*uold(i,j,3)+u(i-1,j,3))/dx^2;     % Second derivative of y velocity w.r.t. x
        d2vdy2 = (uold(i,j+1,3)-2*uold(i,j,3)+u(i,j-1,3))/dy^2;     % Second derivative of y velocity w.r.t. y

        beta2 = max([u(i,j,2)^2 + u(i,j,3)^2, rkappa*vel2ref]);

        u(i,j,1) = uold(i,j,1) - (beta2*dt(i,j))*(rho*dudx + rho*dvdy - (artviscx(i,j)+artviscy(i,j)) - s(i,j,1));
        u(i,j,2) = uold(i,j,2) - (dt(i,j)*rhoinv)*(rho*uold(i,j,2)*dudx + rho*uold(i,j,3)*dudy + dpdx - rmu*(d2udx2 + d2udy2) - s(i,j,2));
        u(i,j,3) = uold(i,j,3) - (dt(i,j)*rhoinv)*(rho*uold(i,j,2)*dvdx + rho*uold(i,j,3)*dvdy + dpdy - rmu*(d2vdx2 + d2vdy2) - s(i,j,3));
    end
end
...
for j = 2:jmax-1
    for i = 2:imax-1
        dpdx = (u(i+1,j,1)-uold(i-1,j,1))/(2*dx);       % First derivative of pressure w.r.t. x
        dpdy = (u(i,j+1,1)-uold(i,j-1,1))/(2*dy);       % First derivative of pressure w.r.t. 

        dudx = (u(i+1,j,2)-uold(i-1,j,2))/(2*dx);       % First derivative of x velocity w.r.t. x
        dudy = (u(i,j+1,2)-uold(i,j-1,2))/(2*dy);       % First derivative of x velocity w.r.t. y

        dvdx = (u(i+1,j,3)-uold(i-1,j,3))/(2*dx);       % First derivative of y velocity w.r.t. x
        dvdy = (u(i,j+1,3)-uold(i,j-1,3))/(2*dy);       % First derivative of y velocity w.r.t. y

        d2udx2 = (u(i+1,j,2)-2*uold(i,j,2)+uold(i-1,j,2))/dx^2;     % Second derivative of x velocity w.r.t. x
        d2udy2 = (u(i,j+1,2)-2*uold(i,j,2)+uold(i,j-1,2))/dy^2;     % Second derivative of x velocity w.r.t. y

        d2vdx2 = (u(i+1,j,3)-2*uold(i,j,3)+uold(i-1,j,3))/dx^2;     % Second derivative of y velocity w.r.t. x
        d2vdy2 = (u(i,j+1,3)-2*uold(i,j,3)+uold(i,j-1,3))/dy^2;     % Second derivative of y velocity w.r.t. y
...
for j = 2:jmax-1
    for i = 2:imax-1
        dpdx = (uold(i+1,j,1)-uold(i-1,j,1))/(2*dx);       % First derivative of pressure w.r.t. x
        dpdy = (uold(i,j+1,1)-uold(i,j-1,1))/(2*dy);       % First derivative of pressure w.r.t. 

        dudx = (uold(i+1,j,2)-uold(i-1,j,2))/(2*dx);       % First derivative of x velocity w.r.t. x
        dudy = (uold(i,j+1,2)-uold(i,j-1,2))/(2*dy);       % First derivative of x velocity w.r.t. y

        dvdx = (uold(i+1,j,3)-uold(i-1,j,3))/(2*dx);       % First derivative of y velocity w.r.t. x
        dvdy = (uold(i,j+1,3)-uold(i,j-1,3))/(2*dy);       % First derivative of y velocity w.r.t. y

        d2udx2 = (uold(i+1,j,2)-2*uold(i,j,2)+uold(i-1,j,2))/dx^2;     % Second derivative of x velocity w.r.t. x
        d2udy2 = (uold(i,j+1,2)-2*uold(i,j,2)+uold(i,j-1,2))/dy^2;     % Second derivative of x velocity w.r.t. y

        d2vdx2 = (uold(i+1,j,3)-2*uold(i,j,3)+uold(i-1,j,3))/dx^2;     % Second derivative of y velocity w.r.t. x
        d2vdy2 = (uold(i,j+1,3)-2*uold(i,j,3)+uold(i,j-1,3))/dy^2;     % Second derivative of y velocity w.r.t. y
...
if n==1
    ...
    resinit(2) = sqrt(sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of xmtm
    resinit(3) = sqrt(sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of ymtm
    ...
    res(2) = sqrt(sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of xmtm
    res(3) = sqrt(sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of ymtm
...
global zero imax jmax neq imms xmax xmin ymax ymin u uold dt n
...
% L1 norm
if n==1
    resinit_1(1) = sum(abs((u(:,:,1)-uold(:,:,1))./dt(:,:)),'all')/(imax*jmax); %L1 norm of continuity
    resinit_1(2) = sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)),'all')/(imax*jmax); %L1 norm of xmtm
    resinit_1(3) = sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)),'all')/(imax*jmax); %L1 norm of ymtm
    ...
    res_1(1) = sum(abs((u(:,:,1)-uold(:,:,1))./dt(:,:)),'all')/(imax*jmax); %L1 norm of continuity
    res_1(2) = sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)),'all')/(imax*jmax); %L1 norm of xmtm
    res_1(3) = sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)),'all')/(imax*jmax); %L1 norm of ymtm
end

% L2 norm
if n==1
    resinit_2(1) = sqrt(sum(abs((u(:,:,1)-uold(:,:,1))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of continuity
    resinit_2(2) = sqrt(sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of xmtm
    resinit_2(3) = sqrt(sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of ymtm
    ...
    res_2(1) = sqrt(sum(abs((u(:,:,1)-uold(:,:,1))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of continuity
    res_2(2) = sqrt(sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of xmtm
    res_2(3) = sqrt(sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)).^2,'all')/(imax*jmax)); %L2 norm of ymtm

end

% Linf norm
if n==1
    resinit_inf(1) = max(abs((u(:,:,1)-uold(:,:,1))./dt(:,:)), [], 'all'); %Linf norm of continuity
    resinit_inf(2) = max(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)), [], 'all'); %Linf norm of xmtm
    resinit_inf(3) = max(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)), [], 'all'); %Linf norm of ymtm

    res_inf(1) = resinit_inf(1);
    res_inf(2) = resinit_inf(2);
    res_inf(3) = resinit_inf(3);

else

    res_inf(1) = max(abs((u(:,:,1)-uold(:,:,1))./dt(:,:)), [], 'all'); %Linf norm of continuity
    res_inf(2) = max(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)), [], 'all'); %Linf norm of xmtm
    res_inf(3) = max(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)), [], 'all'); %Linf norm of ymtm
